...
[abstract]
== Abstract
Synapse is a {CPP} signal programming library. It can be used to define communication channels between objects of arbitrary types in a dynamic object environment.
...
The source code is available in https://github.com/zajo/boost-synapse[this GitHub repository].
...
Synapse requires compiler support for the following {CPP}11 features:
...
See the link:https://travis-ci.org/zajo/boost-synapse[Synapse Travis CI Builds].

== Tutorial

=== Signals, emitters, receivers

Typically, signal programming libraries allow user-defined objects to define _signals_ and to act as signal emitters. Like function types, each signal has a signature. Emitting a signal is similar to a function call, but instead of calling a particular function, it calls all functions that are currently connected to that particular signal from that particular emitter object. Naturally, the signature of any connected function must match the signature of the signal.

In Synapse, signals are defined as function pointer typedefs. When a signal is emitted, the value returned from any connected function is discarded.  This enables the return type of the signal signature to be used as an identifier of the signal. For example, the following typedefs define two different signals, each taking one `int`:

[source,c++]
----
struct sig1_;
typedef sig1_(*sig1)(int);

struct sig2_;
typedef sig2_(*sig2)(int);
----

The two defined signals are different because the return value of their respective signatures are of different type (`sig1_` vs. `sig2_`). By convention, these types are defined implicitly within each `typedef`. This makes the signal definitions more compact, if perhaps less intuitive:

[source,c++]
----
typedef struct sig1_(*sig1)(int);
typedef struct sig2_(*sig2)(int);
----

To emit a Synapse signal, we instantiate the `emit` function template with a signal type, passing the emitter object as the first argument. The rest of the arguments follow, as defined by the signal signature.

As an example, let's define a type `button` that emits a Synapse signal `clicked` (which takes no arguments) when the member function `click` is called:
...
Next, let's connect the signal `button::clicked` to the `accept` member function of a dialog box object:
...
NOTE: The `receiver` argument to `connect` is optional. If it is specified, a pointer to the receiver object is passed as the first argument to the connected function, followed by all other arguments as specified by the signal signature.

=== Emitting signals from objects of 3rd-party types

In Synapse, any object whatsoever can be used as an emitter. This makes it possible to `emit` non-intrusively even if the emitter object was not designed to support signals. For example, a function that processes a file can use a standard `FILE` pointer as a Synapse emitter to report on its progress:
...
- `shared_ptr<<<connection,connection>>>` objects are returned by overloads of <<connect,`connect`>> which take the emitter (and, if specified, the receiver) as a raw pointer. The user is required to keep the `connection` object alive; the signal is disconnected when that object expires.
- `weak_ptr<<<pconnection,pconnection>>>` objects are returned by overloads of <<connect,`connect`>> which take at least one of the emitter or the receiver as a `weak_ptr` or `shared_ptr`. The user is _not_ required to keep `pconnection` objects alive; the connection is disconnected when Synapse detects that the emitter or the receiver have expired.

IMPORTANT: The <<release,`release`>> function can be used to convert a <<pconnection,`pconnection`>> object to a <<connection,`connection`>> object, in case we need to control its lifetime explicitly.

NOTE: Earlier Synapse versions lacked support for `pconnection` objects, meaning the user was always required to manage connections explicitly. Legacy Synapse is available in the `legacy` branch of the https://github.com/zajo/boost-synapse[GitHub repository].
...
int main() {
    ...
    auto c = synapse::connect<my_signal>(&emitter,
        [ ](std::string const & s) {
            std::cout << s;
        } );
...
When a `Signal` is blocked or unblocked, the meta emitter emits the `meta::blocked<Signal>` signal. Connecting this signal allows the blocked state of the signal to be automatically reflected in other systems, for example in user interface.

Similarly, when a `Signal` is connected or disconnected, the meta emitter emits the <<meta::connected,`meta::connected<Signal>`>> signal. This meta signal is useful for integrating Synapse with 3rd-party callback systems; see <<Synapsifying C callbacks>>.

NOTE: To further facilitate the integration of Synapse with 3rd-partty callback systems, <<connection,`connection`>>  objects (returned by the <<connect,`connect`>> function) can store arbitrary user data.
...
It is also possible for any thread to request to receive all signals emitted by other threads, by creating its own <<thread_local_queue,`thread_local_queue`>> object using <<create_thread_local_queue,`create_thread_local_queue`>>.
...
- `emit` takes its arguments by value. Use `boost::ref` to pass by reference (but beware of <<thread_local_queue,`thread_local_queue`>> objects).
...
Overloads of `connect` that return `weak_ptr<<<pconnection,pconnection>>>` take at least one of `e` and `r` as a `weak_ptr` or `shared_ptr`. They create _persistent_ connections which expire when either `e` or `r` expire.
...
+
IMPORTANT: If either the emitter or the receiver, passed as raw pointers, expire before the returned <<connection,`connection`>> object has expired, the behavior is undefined.
+
...
+
NOTE: Because class <<connection,`connection`>> is the protected base of class <<pconnection,`pconnection`>>, handlers of `meta::connected<Signal>` take `connection &` regardless of which `connect` overload was used.
+
...
Overloads of <<connect,`connect`>> and <<translate,`translate`>> return either `shared_ptr<connection>`  or `weak_ptr<pconnection>`, depending on whether or not the emitter and the receiver are passed as raw pointers. The former is used to control the lifetime of the connection explicitly, while the latter represents persisent connections, which expire implicitly with the expiration of the emitter or the receiver object.
...
* If <<set_user_data,`set_user_data`>> has not been called for `this`, or if the type used to instantiate the `set_user_data` function template doesn't match the type used with `get_user_data`, returns `0`.
...
* If the emitter was passed to <<connect,`connect`>> (or <<tranlate,`translate`>>) as a raw pointer, the returned `shared_ptr` points that emitter but does not (can not) keep it alive.
...
* If the receiver was passed to <<connect,`connect`>> (or <<tranlate,`translate`>>) as a raw pointer, the returned `shared_ptr` points that receiver but does not (can not) keep it alive.
...
It is often needed to monitor the operations of a complex dynamic system beyond the facilities available in its formal API. One possible option to accomplish this is to use a logging library. Synapse provides another.
...
Synapse can be used with any C-style callback API to install {CPP} functions, including lambdas, as callbacks. For example, consider the `SSL_set_info_callback` from https://www.openssl.org/[OpenSSL]:
...
We can define the following Synapse signal to represent this callback:
...
Next, at a global scope or during initialization, we install a handler for the `meta::connected<SSL_info_callback>` signal, which the <<meta::connected,`meta::emitter`>> emits every time the user connects or disconnects the `SSL_info_callback` signal:
...
<2> The user is _connecting_ the `SSL_info_callback` signal.
...
<5> The user is _disconnecting_ the `SSL_info_callback` signal.
...
Once the above handler for the `meta::connected<SSL_info_callback>` signal is installed, we can simply use <<connect,`connect`>> to install a {CPP} lamda handler for the `SSL_info_callback` signal we defined:
...
See this https://en.wikipedia.org/wiki/Signals_and_slots#Alternative_implementations[Wikipedia page].
...
----
All assertions in Synapse use this macro; if not `#defined`, Synapse header files `#define` it as link:http://www.boost.org/doc/libs/release/libs/assert/assert.html[`BOOST_ASSERT`].
...
NOTE: The `init` argument passed will contain parentheses, so when the macro is expanded it results in calling a constructor.
...
IMPORTANT: Except under `BOOST_SYNAPSE_NO_THREADS`, initialization of objects with static storage is expected to be serialized.
...
How does Synapse differ from Boost Signals 2, which is also "non-intrusive"?::
    The main design difference is that in Boost Signals 2 emitting a signal requires a signal object. This is significant difference because in general it is not trivial to associate a Boost Signals 2 signal object with 3rd-party objects. When using Synapse such association is not necessary because the 3rd-party object itself can be passed to <<emit,`emit`>>.
...
[.small]#_(C) 2015-2018 Emil Dotchevski_#
