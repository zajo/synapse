...
The cautious reader has surely noticed that in the above example we had to capture the return value of `synapse::connect<report_progress>` in  `c`, while we didn't have to do this in the previous `button`/`dialog` example. This is explained below.
...
- `shared_ptr<<<connection,connection>>>` objects are returned by overloads of <<connect,`connect`>> which take the emitter (and, if specified, the receiver) as a raw pointer. The user is required to keep the `connection` object alive; the function will be disconnected when that object expires.
- `weak_ptr<<<pconnection,pconnection>>>` objects are returned by overloads of <<connect,`connect`>> which take at least one of the emitter or the receiver as a `weak_ptr` or `shared_ptr`. The user is _not_ required to keep `pconnection` objects alive; the function is disconnected when Synapse detects that the emitter or the receiver have expired.

IMPORTANT: The <<release,`release`>> function can be used to convert a <<pconnection,`weak_ptr<pconnection>`>> object to a <<connection,`shared_ptr<connection>`>> object, in case we need to disconnect the function before the receiver or the emitter have expired.
...
Similarly, when a signal `S` is connected or disconnected, the meta emitter emits the <<meta::connected,`meta::connected<S>`>> signal, which is useful when integrating Synapse with 3rd-party callback systems; see <<Synapsifying C callbacks>>.

NOTE: To further facilitate the integration of Synapse with 3rd-partty callback systems, <<connection,`connection`>>/<<pconnection,`pconnection`>>  objects (returned by overloads of <<connect,`connect`>>) can store arbitrary user data.
