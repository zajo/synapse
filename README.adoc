...
For a list of other signal programming libraries, see this https://en.wikipedia.org/wiki/Signals_and_slots#Alternative_implementations[Wikipedia page], however Synapse is unique in that it is non-intrusive with respect to the signal emitter type. While other libraries allow users to create types that can emit signals, Synapse is able to emit any signal from any object of any type whatsoever.
