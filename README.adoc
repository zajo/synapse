...
[abstract]
== Abstract
Synapse is a {CPP} signal programming library.

While other similar libraries allow users to create types that can emit signals, Synapse is able to emit any signal from any object of any type whatsoever. If two contexts in a given program happen to have access to the same object, they can use that object, regardless of its type, to communicate through Synapse signals.

Meta signals can be used to integrate Synapse with 3rd-party APIs. In particular, Synapse enables {CPP} function objects, including lambda functions, to be used as callbacks even with C-style interfaces which only support C function pointer callbacks.
...
The cautious reader has surely noticed that in the above example we had to capture the return value of `synapse::connect<report_progress>` in the local variable  `c`, while we didn't have to do this in the previous `button`/`dialog` example. This is explained below.
...
The unique design feature of Synapse is that it is non-intrusive with respect to the signal emitter type. While other libraries provide users with objects that can emit signals, Synapse is able to emit any signal from any object of any type whatsoever.

Since Synapse is formatted for Boost review, people often ask what are the differences between Synapse and https://www.boost.org/doc/libs/release/doc/html/signals2.html[Boost Signals 2].

.Comparison between Synapse and Signals 2
|====
| | Signals 2     | Synapse
| What is a signal? | An object of type `signal<T>`, which maintains a list of connected functions, and is callable like `std::function<T>`. | A type, similar to a C function pointer `typedef`.
| How do I emit a signal? | Invoking `operator()` on a signal object calls all connected functions. | Calling <<emit,`emit<S>(e)`>> calls all functions connecting the signal `S` for the emitter `e`.
...
| Support for meta signals? | No (not possible, a signal is an object). | Yes. For example, connecting a signal of type `S` emits the signal <<meta::connected,`meta::connected<S>`>> from the <<meta::connected,`meta::emitter`>>.
| Integration with C-style callback APIs? | No (not possible). | Yes, through meta signals, see <<Synapsifying C callbacks>>.
| Thread safety? | Yes, the connection list maintained in each signal object is thread-safe. | Yes, the connection lists are thread-local, and signals are transported to other threads using <<thread_local_queue,`thread_local_queue`>> objects.
|====
...
[qanda]
How does Synapse differ from Boost Signals 2?::
    The main design difference is that in Boost Signals 2 is intrusive; only objects of a specific Boost Signals 2 type can emit signals. This is a significant limitation because in general it is not trivial to associate a Boost Signals 2 signal object with 3rd-party objects. When using Synapse such association is not necessary because the 3rd-party object itself can be passed to <<emit,`emit`>>.
