...
In Synapse, signals are defined by function pointer typedefs. The return type is ignored in function calls. Instead, it is used to tell apart different signals that have otherwise identical signature.
...
- `shared_ptr<<<connection,connection>>>` objects are returned by overloads of <<connect,`connect`>> which take the emitter (and, if specified, the receiver) as a raw pointer. The user is required to keep the `connection` object afloat; the signal is disconnected when it expires.
- `weak_ptr<<<pconnection,pconnection>>>` objects are returned by overloads of <<connect,`connect`>> which take at least one of the emitter or the receiver as a `weak_ptr` or `shared_ptr`. The user is _not_ required to keep `pconnection` objects afloat; the connection is disconnected when Synapse detects that the emitter or the receiver have expired.
...
Overloads of `connect` that return `shared_ptr<<<connection,connection>>>` create connections whose lifetime is explicitly managed by the user. Such connections require that the caller keeps the returned <<connection,`connection`>> object afloat for as long as the connection should persist.
...
* If the emitter was passed to <<connect,`connect`>> (or <<tranlate,`translate`>>) as a raw pointer, the returned `shared_ptr` points that emitter but does not (can not) keep it afloat.
...
* If the receiver was passed to <<connect,`connect`>> (or <<tranlate,`translate`>>) as a raw pointer, the returned `shared_ptr` points that receiver but does not (can not) keep it afloat.
...
Synapse can be used with any C-style callback API to install C++ functions, including lambdas, as callbacks. For example, consider the `SSL_set_info_callback` from https://www.openssl.org/[OpenSSL]:
...
Once the above handler for the `meta::connected<SSL_info_callback>` signal is installed, we can simply use <<connect,`connect`>> to install a C++ lamda handler for the `SSL_info_callback` signal we defined:
...
<2> Needed to keep the Synapse connection afloat.
...
IMPORTANT: Except under `BOOST_SYNAPSE_NO_THREADS`, objects with static storage are expected to be thread-safe.
