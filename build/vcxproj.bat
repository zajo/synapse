...
call "%~dp0..\..\..\b2.exe" address-model=64 toolset=%1 address-model=64 variant=debug "%~n2" -a -n|"%~dp0vcxproj.exe" "%~2" --cfg debug --bjam_log_stdin --out "%~2" --flt_dir \synapse=synapse* --jamfile_dir "%~dp2" %3 %4 %5 %6 %7 %8 %9
if ERRORLEVEL 1 goto :error
call "%~dp0..\..\..\b2.exe" address-model=64 toolset=%1 address-model=64 variant=release "%~n2" -a -n|"%~dp0vcxproj.exe" "%~2" --cfg release --bjam_log_stdin --out "%~2" --flt_dir \synapse=synapse* --jamfile_dir "%~dp2" %3 %4 %5 %6 %7 %8 %9
if ERRORLEVEL 1 goto :error
exit /B 0
