...
[.text-right]
https://github.com/zajo/synapse[GitHub]
endif::[]
...
Signal programming libraries allow _signals_ to be associated with emitter objects. Each signal has a signature. Emitting a signal is similar to function invocation, except it may call multiple functions that currently connect that particular signal from that particular emitter object. Naturally, the signature of any connected function must match the signature of the signal.

In Synapse, signals are defined as empty types derived from `synapse::signal<Signature>`. The derived type identifies the signal, while the base class specifies its signature. This makes it possible to define different signals that have identical function signatures:

[source,c++]
----
struct this_signal: synapse::signal<void(int)> {};
struct that_signal: synapse::signal<void(int)> {};
----

To emit a Synapse signal, we instantiate the `emit` function template with a pre-defined signal type (e.g. `this_signal`), passing the emitter object as the first argument. The rest of the arguments follow, as defined by the signal signature (in this case a single `int` argument). Note that the emitter object passed as the first argument to `emit<S>` is not forwarded implicitly to the connected functions; its only purpose is to specify the _emitter_, that is, which object is emitting the signal `S`.
...
[source,c++]
----
struct clicked: synapse::signal<void()> {};
...
[source,c++]
----
struct report_progress: synapse::signal<void(int)> {};
...
struct print: synapse::signal<void(std::string const &)> {};
...
[source,c++]
----
struct my_signal: synapse::signal<void(int *)> {};
...
Using Synapse, we can easily define a set of signal types to represent such events:

[source,c++]
----
struct object_loaded: synapse::signal<void(char const *, char const *)> {};
struct object_unloaded: synapse::signal<void(char const *, char const *)> {};
struct cache_miss: synapse::signal<void(char const *, char const *)> {};
struct cache_hit: synapse::signal<void(char const *, char const *)> {};
----
...
[source,c++]
----
struct ssl_info_callback: synapse::signal<void(SSL const *, int, int)> {};
----
...
[source,c++]
----
struct exception_caught: synapse::signal<void()> {};
----
...
public:
	...
	struct counterReached: synapse::signal<void()> {}; <1>
...
| What is a signal? | An object of type `signal<T>`, which maintains a list of connected functions, and is callable like `std::function<T>` is. | A type derived from `synapse::signal<T>`.
...
All assertions in Synapse use this macro. If it is not defined, Synapse header files `#define` it as `assert`.
