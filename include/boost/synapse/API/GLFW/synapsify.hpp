...
#include <boost/synapse/API/GLFW/signals.hpp>
...
boost
    ...
    synapse
        ...
        synapsify_glfw
            ...
            class
            synapsifier<R(*)(GLFWwindow *,A...)>
                {
                typedef R(*Signal)(GLFWwindow *,A...);
                typedef void (*GLFWfun)( GLFWwindow *,A... );
                static GLFWfun prev_;
                
                //This is the handler that GLFW calls. It emits the corresponding Synapse signal and calls the
                //previous GLFW handler for the same event, if any.
                static
                void
                handler( GLFWwindow * w, A... a )
                    {
                    try
                        {
                        ...
                        {
                        //We can't let exceptions propagate up into C code, so the window emits the exception_caught
                        //signal, which (if exceptions are expected) should be connected to capture and handle the
                        //current exception.
                ...
                explicit
                synapsifier( GLFWfun (*setter)(GLFWwindow *,GLFWfun) )
                    {
                    //Here we connect the Synapse meta::connected<Signal> signal. This signal is emitted by the
                    //meta::emitter() when the Signal is being connected (the user calls synapse::connect<Signal>) or
                    //disconnected (when the connection expires). The emitter pointer passed to connect (which
                    //in this case is of type GLFWwindow) is stored in the synapse::connection object passed to the
                    //lambda below, and can be accessed by the connection::emitter member function template.
                    connect<meta::connected<Signal> >( meta::emitter(),
                        [setter]( connection & c, unsigned flags )
                            {
                            if( flags&meta::connect_flags::connecting )
                                {
                                //When the Signal is being connected for the first time, use the GLFW API to install our handler.
                                if( flags&meta::connect_flags::first_for_this_emitter )
                                    prev_=setter(c.emitter<GLFWwindow>().get(),&handler);
                                }
                            else
                                {
                                //When the last Signal connection expires, use the GLFW API to uninstall our handler and
