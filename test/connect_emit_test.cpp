...
namespace
    ...
    test_receiver_mem_fn_bind()
        ...
        BOOST_TEST(synapse::emit<test_signal1>(&emitter)==0);
        BOOST_TEST(synapse::emit<test_signal2>(&emitter)==0);
        shared_ptr<synapse::connection const> c1 = synapse::connect<test_signal1>( &emitter, &receiver1, boost::bind(&test_class::fn,_1) );
        BOOST_TEST(c1.unique());
        shared_ptr<synapse::connection> c2 = synapse::connect<test_signal2>( &emitter, &receiver2, boost::bind(&test_class::fn,_1) );
        BOOST_TEST(c2.unique());
        BOOST_TEST(synapse::emit<test_signal1>(&emitter)==1);
        BOOST_TEST(count1==1);
        BOOST_TEST(count2==0);
