...
main( int argc, char const * argv[] )
    ...
    BOOST_TEST(emit_counter1==0);
    BOOST_TEST(emit_counter2==0);
    synapse::emit<my_signal>(e1.get());
    BOOST_TEST(emit_counter1==1);
    BOOST_TEST(emit_counter2==0);
    synapse::emit<my_signal>(e2.get());
    BOOST_TEST(emit_counter1==1);
    BOOST_TEST(emit_counter2==1);

    BOOST_TEST(meta_counter==2);
        {
        BOOST_TEST(!c1.expired());
        BOOST_TEST(!c2.expired());
        shared_ptr<synapse::connection> c=synapse::connect<my_signal>(e1,[ ]( ){ });
        BOOST_TEST(c1.expired());
        BOOST_TEST(c2.expired());
        BOOST_TEST(meta_counter==1);
        }
