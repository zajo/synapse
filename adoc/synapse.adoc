...
<3> At this point calling `emit` is a noop because the connection `c` has expired.
====
...
Similarly, when a `Signal` is connected or disconnected, the meta emitter emits the `meta::connected<Signal>`. This meta signal is useful for integrating Synapse with 3rd-party callback systems; see this example.
...
WARNING: In case `Signal` connections exist in other threads in which the user has created `thread_local_queue` objects, the arguments passed to `emit` are copied and must remain valid until the `Signal` is consumed by `poll`.
...
namespace boost { namespace synapse {
    ...
    <<connection>> connect( <<Emitter>> e, F f ); //<1>

    template <class Signal,class Emitter,class Receiver,class F>
    <<connection>> connect( <<Emitter>> e, <<Receiver>> r, F f ); //<2>

} }
----
<1> `e` is of type `weak_ptr<Emitter> const &`, `shared_ptr<Emitter> const &`, or `Emitter *`. In the latter case the return value type is `shared_ptr<connection>`, otherwise it is `weak_ptr<connection>`.
<2> `r` by analogy. If _both_ `e` and `r` are passed as raw pointers, the return value type is `shared_ptr<connection>`, otherwise it is `weak_ptr<connection>`.
====

Effects: ::

. Connects the specified `Signal` from the emitter `e` to the function object `f`. How and when the signal disconnects depends on which overload of `connect` is used:

* If an overload that returns `shared_ptr<connection>` is used, the signal disconnects when the returned object expires.
* If an overload that returns `weak_ptr<connection>` is used, the signal disconnects when either the emitter `e` expires (if applicable) _or_ the receiver `r` expires (if applicable). To force disconnection, pass the returned object to `release`.

. The `meta::emitter()` emits the `meta::connected<Signal>` signal:
+
[source,c++]
----
namespace boost { namespace synapse {
...
----
+
The `meta::connected<Signal>` signal is also emitted upon disconnection. If disconnection occurs 
+
Handlers of the meta signal take a mutable reference to the `connection` object being created or destroyed, and a second `unsigned` argument, `flags`, which indicates the circumstances under which the meta signal is emitted:

* If the `connection` object is being created, the `connecting` bit is set, otherwise it is clear.
* If this is the first `Sinal` connection being created for the emitter `e`, the `first_for_this_emitter` bit is set.
* If the `connection` object is being destroyed, and this is the last ever `Sinal` connection for the emitter `e`, the `first_for_this_emitter` bit is set.




* The returned `connection` object expires, at which point the `meta::emitter` emits `meta::connected<Signal>` again, indicating that the connection is being destroyed;
* The emitter expires;
* The receiver expires.

WARNING:  If connect is passed a raw pointer, deleting the emitter before the returned connection object has expired results in undefined behavior.

Notes:

    The returned connection object does not assume ownership of e: passing e as a shared_ptr is equivalent to passing a weak_ptr;
    Use connection::emitter to access e at a later time.
...
Effects: ::
Blocks the specified `Signal` from the emitter `e` until the returned `blocker` object expires. While the `Signal` is blocked, calls to `emit<Signal>` for `e` are ignored and return `0`.
...
- The returned `blocker` object does not own `e` even if the `shared_ptr` block overload is used.
...
This meta signal is emitted by the `meta::emitter` when another signal is blocked or unblocked:

- If `Signal` was not blocked for `e` at the time `block<Signal>` is called, the `meta::emitter` emits the signal `meta::blocked<Signal>`, passing `true` for the `is_blocked` argument;
- When the returned `blocker` object expires, if there are no other `blocker` objects blocking `Signal` for the same emitter `e`, the `meta::emitter` emits the signal `meta::blocked<Signal>` again, with `false` passed for the `is_blocked` argument;

NOTE: The `blocker` object is passed to handlers of `meta::blocked<>`. They can use it to access the emitter `e` passed to `block`.
...
[source,c++]
----
return connect<OriginalSignal>(
    original_emitter, bind(&emit<TranslatedSignal>,translated_emitter,_1,_2,...),connection_lifetime)
----

This function can be used to translate one signal into another signal, as long as their arguments are compatible. For example, if we have a button btn and a dialog dlg, translate<button_on_click,dialog_confirmed>(btn,dlg) can be used to confirm the dialog when the button is clicked.
Note:

The returned connection itself does not count towards the return value of emit<OriginalSignal>; that is, the return value of emit<OriginalSignal> is as if emit<TranslatedSignal> was called instead.
...
The `block` function returns `shared_ptr<blocker const>` that is used to control the time the signal remains blocked. As well, `blocker` objects are passed to handlers of the `meta::blocked` signal, which can use the `emitter` member function template to access the emitter object passed to `block`
...
Returns: ::
A `shared_ptr` that points the emitter that was passed to an overload of the `block` function template to get the `blocker` object.
...
* If the emitter was passed to `block` as a raw pointer, the returned `shared_ptr` points that emitter but does not (can not) keep it afloat.
...
The connect and translate functions return shared_ptr<connection> that is used to control the lifetime of the connection. As well, connection objects are passed to handlers of the meta::connected signal, which can use the emitter member function template to access the emitter object passed to connect/translate, and the connection::set_user_data/connection::get_user_data member function templates to store auxiliary information in the connection object.
...
In synapse, the signal types used to instantiate the `emit()`  and `connect()` function templates must be function pointer types. The signal signature is used to deduce the arguments of `emit()`, and must be compatible with the function argument of `connect()`.
...
In addition to function pointer types, the `emit()` and `connect()` templates may be instantiated with instances of the `meta::connected<>` and `meta::blocked<>` class templates. These meta signal templates must be instantiated with another signal type (that is, a function pointer type or an instance of `meta::connected<>` or `meta::blocked<>`).
