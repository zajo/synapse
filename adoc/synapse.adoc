...
- `shared_ptr<<<connection,connection>>>` objects are returned by overloads of <<connect,`connect`>> which take the emitter (and, if used, the receiver) as a raw pointer. The user is required to keep this object afloat; the signal is disconnected when it expires.
